...
class BookClassEvent extends Conversation
{
    ...
    {
        $this->ask('Hi 👋, Welcome to the Chat n Pay booking assistant. Kindly respond with *Studio Classes* or *Hikes* to checkout upcoming events.', function(Answer $answer){
                ...
                $this->getClassOrEvents('Event');
                    ...
                    return $this->repeat('_'.$this->askService.'_ doesn\'t look like a valid respond. Please reply with either *Studio Classes* or *Hikes*.');                  
                }
        });
    }
    ...
    public function getClassOrEvents($classOrEvent)
    {
        ...
        $events =  DB::table('class_events')
                    ...
                    foreach ($events as $event) {
                        $events_text .=$counter.'. *'.$event->code.'*  '.$event->title.' at '.$event->venue.'.  ';
                        $counter++;
                    }
        ...
        if ($eventsCount >= 1) {
            $this->ask('We have the following '.$classOrEvent.'s lined up: '.$events_text.'Kindly respond with the respective 👆 code to book.' , function(Answer $answer){
                ...
                if ( preg_match('/^[A-Z]{3}-{1}\d{2}$/i', strtoupper($this->classEventCode))){
                    ...
                    return $this->repeat(' 👶 Ooops I couldn\'t understand _'.$this->classEventCode.'_ . Please check the submitted phrase matches the respective event code.');
                }       
            });             
        } else {
            $this->ask('👨‍💻 Ooops looks like my admin is currently updating _'.$classEvent.'s_ . Please check back after an hour 👋. Looking forward to our chat. Thank you' , function(Answer $answer){ 
    ...
    public function confirmEvent($code)
    {
            ...
            $classEventCollection = DB::table('class_events')
                                    ...
                                    ->first();
            $this->ask('To book a spot for: *'.$classEventCollection->code.'* '.$classEventCollection->title.' at '.$classEventCollection->venue.' for Kshs. '.$classEventCollection->cost .' _Description_ '.$classEventCollection->desc.' kindly respond with the MPESA number to receive an STK Push. Accepatble _Number format_ is *2547XXXXXXXX* .' , function(Answer $answer){
                ...
                $this->mpesaNumber = $answer->getText();
                    if (preg_match('/^[2]{1}[5]{1}[4]{1}[0-9]{9}$/i', $this->mpesaNumber)) {
                        ...
                        return $this->repeat('Something is not correct with the shared number ( _'.$this->mpesaNumber.'_  ). Please check on that and try again. NB use a *2547XXXXXXXX* Safaricom registered number.');
                    }
    ...
    public function askNumberOfSlots($bookID)
    {
        ...
        $this->ask('We are almost done 🤓. How many slots would you like to reserve? *NB* you are allowed to reserve a min of 1 and a max of 15 slots. ', function(Answer $answer){
            $this->slots = $answer->getText();
             if (($this->slots >= 1) && ($this->slots <= 15) ) {
                    ...
                    return $this->repeat(' 👶 _'.$this->slots.'_ does not fall in the range defined above 👆 rule. Kindly enter a number (1-15)');
                }    
        });
    }
...
public function askPayment($bookID)
{
    ...
    $this->ask($updateBooking->slots.' slot(s) for '.$event->title.' will cost Kshs. '.$updateBooking->amount. '. Please enter your MPESA _pin_ on the STK pop dialog to checkout. I will take several seconds break as we wait for MPESA confirmation.', function(Answer $answer){
